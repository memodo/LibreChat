...
- **Capability functional smoke tests (2026-07-15)** — the 3 newly-adopted, prod-bound capabilities exercised for real (never functionally tested before):
  ...
  - [x] **chain VERIFIED (Agent Chain / Mixture-of-Agents).** Built a 2-agent chain: primary `Chain Test Primary` ("answer in one short sentence") → chained `Uppercase Bot` ("uppercase the previous message"). Asked "What is the capital of France?" → output showed **both** steps in sequence: primary `Sir, the capital of France is Paris.` then `AI: SIR, THE CAPITAL OF FRANCE IS PARIS.` — the second agent provably consumed the first's output (Advanced → Chain via `agent_ids`, gated by the `chain` capability). No server errors. _(browser)_
  - **All 3 newly-adopted capabilities (skills/memory/chain) now functionally smoke-tested — round COMPLETE.**
