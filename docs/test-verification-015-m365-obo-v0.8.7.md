...
- [~] **OneDrive / Files — auth VERIFIED; listing has an OPEN defect (Finding #5).** 2026-07-13: "List my 5
  most recent OneDrive files" → real files returned; OBO/auth fine. **2026-07-15 live retest (at `0194171d4`):**
  Finding #2 steer VERIFIED — the model now uses `list-drives → get-drive-root-item → list-folder-files`
  with **no `q="*"` 400** (contrast the 2026-07-13 log, which shows the old `search-onedrive-files` q=a/e/o
  letter-guessing). **But** the model picked the WRONG drive (`VivaEngage`, a service library) and reported
  its lone folder as "your files" — the real OneDrive is the `Dokumente` drive (`driveType: business`).
  Targeting `Dokumente` explicitly returned the real ~20+ items with paging. → **Finding #5 (wrong-drive
  selection), OPEN**; fix = v8 serverInstructions steer to select the `business`/`personal` drive. _(browser + log)_
