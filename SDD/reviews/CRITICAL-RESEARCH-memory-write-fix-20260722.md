...
### Live Confirmation (2026-07-23) — the runtime-only question resolved, and it was neither reviewer's nor the research's leading suspect

The Option A1 supervised live repro this review's Finding 6 and Recommended Action 3 called for was executed on 2026-07-23. It resolved the residual runtime-only question this review correctly identified as the last open item — but the resolution was **not** a tool-calling failure at all, and none of the candidates this review or the research document discriminated between (H1 silent no-op, H2 inline-capability toggle, `397ddc536`, `8fc231420`, `1b79e0b78`/`normalizeMemoryLLMConfig`) turned out to be the cause.

**Actual root cause:** `isMemoryAgentEnabled()` (`packages/data-schemas/src/app/memory.ts:37-40`) requires `config.agent.enabled === true`, a v0.8.7 opt-in gate this fork's `librechat.yaml` never set. With the gate `false`, `useMemory()` never builds the extraction agent or assigns `this.processMemory` in the first place — there was no tool call to trace, no `finalLLMConfig` to strip, and no post-turn agent execution to observe, because the mechanism this review's Findings 1–6 spent their effort discriminating within never ran at all.

This confirms the review's own Finding 4 concern (the "72h zero logs" claim was corroborating, not load-bearing) was directionally correct for a different reason than suspected: the log scan found nothing because the one relevant log line is a one-time startup warning naming the missing `enabled` key, not a per-turn error the scan's grep pattern would have caught regardless of level or path coverage. The review's static work (Findings 2, 3, 7, 8; the `397ddc536`/`8fc231420`/`1b79e0b78` clearances) remains valid and non-wasted — those really are cleared, just not because they were ever reached. See `SDD/research/RESEARCH-019-memory-write-fix.md` ("Root Cause — CONFIRMED (2026-07-23)" and "Live Confirmation (2026-07-23)") and `SDD/requirements/SPEC-019-memory-write-fix.md` for the full trace, the before/after DB evidence (chat-test `memoryentries`: 14→16 rows), and the one-line fix (`memory.agent.enabled: true`).
