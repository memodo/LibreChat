...
**When to use which** — `rebuild-local.sh` and `prod-sync.sh` look similar but target different goals:

- **Prod-only deploy (no local test needed):** `npm run build && ./prod-sync.sh`. `prod-sync.sh` only cares that the `dist/` dirs exist locally; it doesn't touch your local docker.
- **Want to validate locally first, then deploy:** `./rebuild-local.sh` (bounces local docker so you can test at `http://localhost:3080`) → in a second terminal, `./prod-sync.sh` (dists are already fresh from the rebuild, so no second build needed).
- **Config-only changes (`librechat.yaml`, etc.):** Skip both. `git push` + on prod `git pull` + `./prod.sh restart api`.

Don't chain them with `&&` — `rebuild-local.sh` ends with a foreground `docker compose up` that blocks the script.
...
| `./prod-sync.sh`                                                                                          | May need `npm run build`. rsync locally-built `packages/*/dist`, `client/dist`, and `api/server` to the production host, then restart the api container. Supports `--dry-run` and `--no-restart`                         |
| `./rebuild-local.sh`                                                                                   | Stop containers → `npm run smart-reinstall` (install if lockfile changed, then Turborepo build of all workspaces) → restart containers. Use after editing any `packages/*/src/` or `client/src/`                        |
